...
//Função criada para desenhar quadrados/retângulos
...
void Desenha(void){
    ...
    glMatrixMode(GL_PROJECTION); //Previne a modificação de tamanho da figura ao redimensionar a tela
    glLoadIdentity();
    ...
    glBegin(GL_QUADS); //Inicia o desenho dos heróis
...
void Teclado(unsigned char tecla, int x, int y) //Executa ações referentes as entradas do teclado
{
    switch (tecla) {
        case 'T':
        case 't':
...
void TecladoUp(unsigned char tecla, int x, int y) //Executa ações referentes as entradas do teclado
{
    switch (tecla) {
        case 'T':
        case 't':
            Desenha(); //Chama a função que redesenha na tela
            break;
    }
}
...
void init(){
    ...
    glutCreateWindow("Capetão América VS. Homem de Lata"); //Cria a janela
    glutDisplayFunc(Desenha); //Função que é chamada toda vez que houver uma iteração na tela
    glutKeyboardFunc(Teclado); //Função que controla quando uma tecla for pressionada
    glutKeyboardUpFunc(TecladoUp); //Função que controla quando uma tecla for solta
