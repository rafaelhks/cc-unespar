...
GLfloat lightX = 8, lightY = 10, lightZ = 0;
...
void Light(){
    GLfloat luzAmbiente[4]={0.2,0.2,0.2,1};
    ...
    spotlight_direction[0] = 0;
    spotlight_direction[1] = -1;
    spotlight_direction[2] = 0;
...
void menu(){
    ...
    cout<<"Wheel Click + Move - Zoom In/Out"<<endl<<endl;
    cout<<"-------Light Control-------"<<endl;
    cout<<"Teclas de 1 a 5: ilumina o objeto referente"<<endl;
}
...
void Keyboard(unsigned char key, int x, int y){
    ...
    else if(key=='1') lightX = 8;
    else if(key=='2') lightX = 4;
    else if(key=='3') lightX = 0;
    else if(key=='4') lightX = -4;
    else if(key=='5') lightX = -8;
    else cout<<"Comando invalido!"<<endl;
    Light();
}

void KeyboardSpecial(int key, int x, int y){
    if(key==GLUT_KEY_LEFT) lightX++;
    else if(key==GLUT_KEY_RIGHT) lightX--;
    else if(key==GLUT_KEY_UP) lightZ++;
    else if(key==GLUT_KEY_DOWN) lightZ--;
    else cout<<"Comando invalido!"<<endl;
    Light();
}
...
void init(){
    ...
    glutCreateWindow("Computação Gráfica - Prova 2º Bimestre - Exercício 03"); //Cria a janela
