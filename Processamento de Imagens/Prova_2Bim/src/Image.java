    ...
    public Image(Pixel[][] pixels, int height, int width, int type, String path){
        ...
        for (int h = 0; h < height; h++) {
            for (int w = 0; w < width; w++) {
                this.pixels[h][w] = new Pixel(h,w,pixels[h][w].getRGB());
            }
        }
    }
    ...
    public Image(Mat m){
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                this.pixels[i][j] = new Pixel(i,j,m.get(i, j));
            }
        }
    }
    
    public Image subMat(int h,int w,int height,int width){
        Image cut = new Image(height, width, this.type);
        for (int i = h; i < height; i++) {
            for (int j = w; j < width; j++) {
                System.out.println(this.pixels[i][j]);
                cut.put(h, w, this.pixels[i][j]);
            }
        }
    ...
    public void Fill(double[] RGB) {
        for (int i = 0; i < this.height; i++) {
            for (int j = 0; j < this.width; j++) {
                this.pixels[i][j] = new Pixel(i,j,RGB);
            }
        }
    }
...
//    }
    ...
    public static Image getCopy(Image src){
        return new Image(src.getPixels(), src.height(), src.width(), src.type(), src.path());
    }
}